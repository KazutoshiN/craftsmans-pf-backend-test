module V1
  class StatusChangeEventController < ApplicationController
    before_action :set_record, only: %i[show update destroy]

    # GET /records
    def index
      @records = nil
      @records = if params[:issue_id].present?
                   StatusChangeEvent.where(issue_id: params[:issue_id])
                 else
                   StatusChangeEvent.all
                 end
